    ...
    if (api_linit_status_btn != null) {
      ...
      if (api_limit_obj.time_line.remaining != null) {
        timeline_limit_percentage =
          (api_limit_obj.time_line.remaining / api_limit_obj.time_line.limit) *
          100;
        api_limit_dsc_obj.time_line = `タイムライン:${
          api_limit_obj.time_line.remaining
        }/${api_limit_obj.time_line.limit}-${unix_time_mmss(
          api_limit_obj.time_line.reset_unix_time
        )}\r\n`;
      } else {
        //初期状態
      }
      if (api_limit_obj.recommend_timeline.remaining != null) {
        recommend_timeline_limit_percentage =
          ...
          100;
        api_limit_dsc_obj.recommend_timeline = `タイムライン(おすすめ):${
          api_limit_obj.recommend_timeline.remaining
        }/${api_limit_obj.recommend_timeline.limit}-${unix_time_mmss(
          api_limit_obj.recommend_timeline.reset_unix_time
        )}\r\n`;
      } else {
        //初期状態
      }
      if (api_limit_obj.search.remaining != null) {
        search_limit_percentage =
          (api_limit_obj.search.remaining / api_limit_obj.search.limit) * 100;
        api_limit_dsc_obj.search = `検索:${api_limit_obj.search.remaining}/${
          api_limit_obj.search.limit
        }-${unix_time_mmss(api_limit_obj.search.reset_unix_time)}`;
      } else {
        //初期状態
      }
      api_linit_status_btn.textContent = `${Math.floor(
        Math.min(
          timeline_limit_percentage,
          recommend_timeline_limit_percentage,
          search_limit_percentage
        ...
        if (value.opd_settings == undefined || ext_update_flag == true) {
          ...
          if (profile_store[last_load_profile]?.profile == undefined) {
            ...
            chrome.storage.local.set(
              { opd_settings: JSON.stringify(recovery_setting) },
              function () {
                alert(
                  "破損した設定データの自動修復を行いました\r\n再読み込みします"
                );
                last_load_profile = 0;
                window.reload();
              }
          ...
          ext_settings = {
            column_settings: profile_store[last_load_profile].profile,
          };
        } else {
          //ext_settings = JSON.parse(value.opd_settings);
          if (profile_store[last_load_profile]?.profile == undefined) {
...
function run(settings) {
  ...
  document.querySelector("head").insertAdjacentHTML(
    ...
    .dsp_btn_add_post_img{
        ...
        background-image: url(${chrome.runtime.getURL(
          ui_icon_define.add_post_column
        )});
        height: 80%;
        width: 80%;
    }
    .dsp_btn_add_tl_img{
        ...
        background-repeat: no-repeat;
        background-image: url(${chrome.runtime.getURL(
          ui_icon_define.add_timeline_column
        )});
        height: 80%;
        width: 69%;
    }
    .dsp_btn_add_ntfc_img{
        ...
        background-repeat: no-repeat;
        background-image: url(${chrome.runtime.getURL(
          ui_icon_define.add_notification_column
        )});
        height: 80%;
        width: 80%;
    }
    .dsp_btn_add_explr_img{
        ...
        background-repeat: no-repeat;
        background-image: url(${chrome.runtime.getURL(
          ui_icon_define.add_explore_column
        )});
        height: 80%;
        width: 80%;
    }
    .dsp_btn_second_rack_img{
        ...
        background-repeat: no-repeat;
        background-image: url(${chrome.runtime.getURL(
          ui_icon_define.column_second_rack
        )});
        height: 80%;
        width: 80%;
    }
    ...
    .dsp_column_settings_btn{
        display: block;
        background-image: url(${chrome.runtime.getURL(
          ui_icon_define.column_settings
        )});
        background-size: cover;
        width: 20px;
        height: 20px;
  ...
  let default_element = {
    ...
    empty_column: {
      html: `<section draggable="false" id="column_%column_num%" class="dsp_column_draggable_false dsp_column dsp_column_emptycolumn"><div opd_column_type="empty_column" opd_column_width="%column_width_num%" style="height: 100%;min-width: 30rem;display: flex;align-items: center;justify-content: center;"><div><img src="${chrome.runtime.getURL(
        ui_icon_define.column_add_1
      )}" style="filter: brightness(0) saturate(100%) invert(61%) sepia(13%) saturate(13%) hue-rotate(335deg) brightness(89%) contrast(79%);"><p>左のバーからカラムを追加</p></div></div></section>`,
    },
    post: {
    ...
    second_empty_column: {
      html: `<section draggable="false" id="column_%column_num%" class="dsp_column_draggable_false dsp_column dsp_column_second_emptycolumn"><div opd_column_type="second_empty_column" opd_column_width="%column_width_num%" style="height:100%;min-width: 30rem;overflow: hidden;display: flex;align-items: center;justify-content: center;"><div><img src="${chrome.runtime.getURL(
        ui_icon_define.column_add_2
      )}" style="filter: brightness(0) saturate(100%) invert(61%) sepia(13%) saturate(13%) hue-rotate(335deg) brightness(89%) contrast(79%);"><p>1段目のカラムが配置できます</p></div></div></section>`,
    },
    home: {
      ...
      if (
        ...
        if (first_column_end == true) {
          second_column_html += default_element[
            ...
            .replace("%column_top_bar_ch%", init_top_visible_checked)
            .replace("%column_tw_view_mode%", tw_view_type)
            .replace("%column_pinned_ch%", init_pinned_checked)
            .replaceAll("%column_pinned_save_path%", init_pinned_path)
            .replaceAll("%column_save_title%", init_column_save_title)
            .replaceAll("%column_width_num%", column_width_init)
            .replaceAll("%column_auto_reload_ch%", init_auto_reload_checked)
            .replaceAll("%column_auto_reload_time%", auto_reload_time);
        } else {
        ...
        if (
          ...
          settings.column_settings[index].type == "second_empty_column"
        ) {
  ...
  if (first_column_end == true && second_column_end == true) {
    ...
    document.querySelector(
      ".dsp_btn_second_rack_img"
    ).style.backgroundImage = `url(${chrome.runtime.getURL(
      ui_icon_define.column_single_rack
    )})`;
  }
  //
        ...
        .addEventListener("click", function () {
          ...
          for (
            ...
            switch (preload_array[preload_index].type) {
              ...
              case "explore":
                preload_desc_array.push(
                  `${preload_desc_count}-[${preload_array[preload_index].column_save_title}]-ユニバーサル(Explore)カラム`
                );
                break;
              case "misskey":
    ...
    for (let index = 0; index < column_object.length; index++) {
        ...
        if (this.getAttribute("opd_iframe_width_only") != "") {
          ...
          if (opd_column_top_visible_checkbox?.checked != true) {
            if (
              ...
              this.contentWindow.document.querySelector(
                "head style[opd_top_visible_css]"
              ).textContent = `div[data-testid="primaryColumn"]>[tabindex="0"][aria-label]>div:nth-child(1)div[data-testid="primaryColumn"]>[tabindex="0"][aria-label]>div:nth-child(1)`;
            } else {
              if (
                this.closest("div[opd_column_type]").getAttribute(
                  "opd_column_type"
                ...
                this.contentWindow.document.querySelector(
                  "head style[opd_top_visible_css]"
                ).textContent = `div[data-testid="primaryColumn"]>[tabindex="0"][aria-label]>div:nth-child(1){display:none;} div[role="progressbar"] + div{display:none;}`;
              } else {
                this.contentWindow.document.querySelector(
                  "head style[opd_top_visible_css]"
                ).textContent = `div[data-testid="primaryColumn"]>[tabindex="0"][aria-label]>div:nth-child(1){display:none;}`;
              }
            }
          } else {
            //console.log("else")
          ...
          switch (opd_column_tw_view_mode_opt.value) {
            ...
            case "1":
              this.contentWindow.document.querySelector(
                "head style[opd_tw_view_mode_css]"
              ).textContent = `div[data-testid="cellInnerDiv"]:has(div[aria-labelledby]){visibility: hidden; height: 0;}`;
              break;
            case "2":
              this.contentWindow.document.querySelector(
                "head style[opd_tw_view_mode_css]"
              ).textContent = `div[data-testid="cellInnerDiv"]:not(:has(div[aria-labelledby])){visibility: hidden; height: 0;}`;
              break;
            default:
              this.contentWindow.document.querySelector(
                "head style[opd_tw_view_mode_css]"
          ...
          if (
            ...
            const post_column_window =
              opd_column_div.querySelector("iframe").contentWindow;
          }
        }
      });
      //各カラム読み込み後の動作(init)
      column_object[index].addEventListener(
        ...
        function () {
          ...
          let opd_column_auto_reload_time_reload = opd_column_div.querySelector(
          ...
          if (mode != "session_set") {
            ...
            if (column_type === "home" || column_type === "explore") {
              ...
              column_content_reload.Init(
                this.closest("div[opd_column_type]").querySelector("iframe")
                  .contentWindow
              );
            }
          }
          ...
          if (mode != "session_set") {
            opd_column_div
              .querySelector(".dsp_column_settings_panel_close_btn")
              .addEventListener("click", function () {
                const settings_panel = this.closest(
                  "div[opd_column_type]"
                ).querySelector(".dsp_column_settings_panel");
          ...
          if (opd_column_width_select != null) {
            ...
            if (mode != "session_set") {
              opd_column_width_select.addEventListener("change", function () {
                ...
                this.closest("div[opd_column_type]").setAttribute(
                  "opd_column_width",
                  preset_rem
                );
                this.closest(
            ...
            opd_column_width_btn.addEventListener("click", function () {
              ...
              if (setting_width != null) {
                ...
                if (setting_width_num != NaN && setting_width_num > 11) {
                  ...
                  this.closest(
                    "div[opd_column_type]"
                  ).style.width = `${setting_width_num}rem`;
                  column_settings_save("", last_load_profile);
          ...
          if (this.getAttribute("opd_iframe_width_only") != "") {
            ...
            if (opd_column_top_visible_checkbox?.checked != true) {
              ...
              if (
                ...
                this.contentWindow.document.querySelector(
                  "head style[opd_top_visible_css]"
                ).textContent = `div[data-testid="primaryColumn"]>[tabindex="0"][aria-label]>div:nth-child(1){visibility: hidden; height: 0;top: calc(100vh - 60px);position: sticky;backdrop-filter: blur(0px) !important;}[data-testid="app-bar-back"]{visibility: visible; filter: none;}`;
              } else {
                if (
                  this.closest("div[opd_column_type]").getAttribute(
                    "opd_column_type"
                  ...
                  this.contentWindow.document.querySelector(
                    "head style[opd_top_visible_css]"
                  ).textContent = `div[data-testid="primaryColumn"]>[tabindex="0"][aria-label]>div:nth-child(1){visibility: hidden; height: 0;top: calc(100vh - 60px);position: sticky;backdrop-filter: blur(0px) !important;}[data-testid="app-bar-back"]{visibility: visible; filter: none;} div[role="progressbar"] + div{display:none;}`;
                } else {
                  this.contentWindow.document.querySelector(
                    "head style[opd_top_visible_css]"
                  ).textContent = `div[data-testid="primaryColumn"]>[tabindex="0"][aria-label]>div:nth-child(1){visibility: hidden; height: 0;top: calc(100vh - 60px);position: sticky;backdrop-filter: blur(0px) !important;}[data-testid="app-bar-back"]{visibility: visible; filter: none;}`;
            ...
            if (opd_column_auto_reload_checkbox != null) {
              ...
              if (mode != "session_set") {
                opd_column_auto_reload_time_reload.addEventListener(
                  "change",
                  function () {
                    const auto_reload_time = auto_reload_target_elem
                      .closest("div[opd_column_type]")
                      .querySelector(".opd_a_reload_time_setting");
                    if (Number(auto_reload_time.value) >= 1) {
                      alert(
                        `自動更新の秒数を${auto_reload_time.value}秒に設定しました`
                      );
                      column_settings_save("", last_load_profile);
                    } else {
              ...
              if (opd_column_auto_reload_checkbox.checked) {
                ...
                const auto_reload_time_input = auto_reload_target_elem
                  .closest("div[opd_column_type]")
                  .querySelector(".opd_a_reload_time_setting");
                ...
                auto_reload_int = setInterval(function () {
                  ...
                  const path_name =
                    auto_reload_target_elem.contentWindow.location.pathname;
                  if (
                    ["/home", "/search"].includes(path_name) ||
            ...
            if (mode != "session_set") {
              ...
              opd_column_banner_checkbox?.addEventListener(
                ...
                function () {
                  ...
                  let banner_mode_target_object = this.closest(
                    "div[opd_column_type]"
                  ).querySelector("iframe");
                  ...
                  if (this.checked != true) {
                    ...
                    banner_mode_target_object.contentWindow.document.querySelector(
                      "head style[opd_banner_css]"
                    ).textContent = `header[role="banner"]{visibility: hidden; width: 0;};`;
                  } else {
                    //console.log("else")
                    banner_mode_target_object.contentWindow.document.querySelector(
                      "head style[opd_banner_css]"
              ...
              opd_column_top_visible_checkbox?.addEventListener(
                ...
                function () {
                  column_settings_save("", last_load_profile);
                  let topvisible_mode_target_object = this.closest(
                    "div[opd_column_type]"
                  ).querySelector("iframe");
                  ...
                  if (this.checked != true) {
                    ...
                    if (
                      ...
                      topvisible_mode_target_object.contentWindow.document.querySelector(
                        "head style[opd_top_visible_css]"
                      ).textContent = `div[data-testid="primaryColumn"]>[tabindex="0"][aria-label]>div:nth-child(1){visibility: hidden; height: 0;top: calc(100vh - 60px);position: sticky;backdrop-filter: blur(0px) !important;}[data-testid="app-bar-back"]{visibility: visible;}`;
                    } else {
                      ...
                      if (
                        ...
                        topvisible_mode_target_object.contentWindow.document.querySelector(
                          "head style[opd_top_visible_css]"
                        ).textContent = `div[data-testid="primaryColumn"]>[tabindex="0"][aria-label]>div:nth-child(1){visibility: hidden; height: 0;top: calc(100vh - 60px);position: sticky;backdrop-filter: blur(0px) !important;} [data-testid="app-bar-back"]{visibility: visible;} div[aria-label="ホームタイムライン"] * +div:first-of-type [data-testid="cellInnerDiv"]{} div[role="progressbar"] + div{display:none;}`;
                      } else {
                        topvisible_mode_target_object.contentWindow.document.querySelector(
                          "head style[opd_top_visible_css]"
                        ).textContent = `div[data-testid="primaryColumn"]>[tabindex="0"][aria-label]>div:nth-child(1){visibility: hidden; height: 0;top: calc(100vh - 60px);position: sticky;backdrop-filter: blur(0px) !important;}[data-testid="app-bar-back"]{visibility: visible;}`;
            ...
            if (opd_column_auto_reload_checkbox != null) {
              if (mode != "session_set") {
                opd_column_auto_reload_checkbox.addEventListener(
                  "click",
                  function () {
                    let auto_reload_target_object = this.closest(
                      "div[opd_column_type]"
                    ).querySelector("iframe");
                    const auto_reload_time_input = this.closest(
                      "div[opd_column_type]"
                    ).querySelector(".opd_a_reload_time_setting");
                    ...
                    if (this.checked) {
                      ...
                      auto_reload_int = setInterval(function () {
                        ...
                        const path_name =
                          auto_reload_target_object.contentWindow.location
                            .pathname;
                        if (
                          ["/home", "/search"].includes(path_name) ||
                          path_name.startsWith("/i/lists")
            ...
            if (mode != "session_set") {
              ...
              opd_column_tw_view_mode_opt.addEventListener(
                ...
                function () {
                  column_settings_save("", last_load_profile);
                  //console.log(this.closest("div[opd_column_type]").querySelector("iframe"))
                  let tw_view_mode_target_object = this.closest(
                    "div[opd_column_type]"
                  ).querySelector("iframe");
    ...
    exp_object.addEventListener("load", function () {
      ...
      let exp_observer = new MutationObserver(function () {
        if (exp_old_url != exp_object.contentWindow.location.href) {
          ...
          let exp_title = exp_object.contentWindow.document.title.replace(
            " / X",
            ""
          );
  ...
  document.getElementById("second_rack").addEventListener("click", function () {
    if (second_rack_mode == false) {
      ...
      const second_rack_default_html = default_element.second_empty_column.html
        .replaceAll("%column_num%", create_random_id())
              ...
              chrome.storage.local.get("opd_settings", function (load_value) {
                ...
                chrome.storage.local.set(
                  ...
                  function () {
                    ...
                    document.querySelector(".profile_val_now").textContent =
                      after_profile_num;
                    document.querySelector("#profile_btn_list").innerHTML =
                      profile_list_btn_html;
                    create_profile_list_btn();
  ...
  function column_dd() {
    ...
    for (let index = 0; index < column_class.length; index++) {
      ...
      column_class[index].addEventListener("drop", function (ev) {
        ...
        if (dr_elem != null) {
          if (
            dr_elem?.querySelector("div")?.getAttribute("opd_column_type") ==
            "explore"
          ) {
            ...
            if (
              dr_elem.querySelector("div").getAttribute("opd_pinned_path") != ""
            ) {
              //console.log("Pinned")
              dr_elem
                .querySelector("div")
                .querySelector("iframe").src = `https://x.com${dr_elem
                .querySelector("div")
  ...
  function column_settings_save(mode, profile_num) {
    ...
    for (
      ...
      if (
        document
          .querySelectorAll("#opd_main_element div[opd_column_type]")
          [index].querySelector(".opd_tw_view_mode")?.value != undefined
      ) {
      ...
      if (
        ...
        if (
          document
            .querySelectorAll("#opd_main_element div[opd_column_type]")
            [index].querySelector(".opd_a_reload_bar")?.checked == true
        ) {
        ...
        const column_setting_time =
          Number(
            document
              .querySelectorAll("#opd_main_element div[opd_column_type]")
              [index].querySelector(".opd_a_reload_time_setting").value
          ) * 1000;
  ...
  const head_observer = new MutationObserver(function () {
    ...
    if (currentScheme) {
      document
        .getElementById("opd_main_element")
        .setAttribute("opd-dsp-theme", currentScheme);
    }
